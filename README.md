...
- Cadastro Beta Android via backend, tabela `beta_testers_android` e status `pending_approval` para aprovação manual
- PWA gerada pelo app Expo em `frontend` com manifest e service worker preparados no build web

## Beta Android

O formulário da landing chama `POST /api/beta/android`. O backend valida consentimento e e-mail, evita duplicidade, salva em `beta_testers_android` com status `pending_approval` e retorna sucesso para a landing. A aprovação do e-mail na lista de testes é manual/externa ao signup.

Antes de liberar acesso aos usuários aprovados, vincule a lista usada para testers à track de teste no Play Console.
